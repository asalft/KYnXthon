    ...
    def jmth(func):
        async def wrapper(event):
            ...
            if from_users and event.sender_id not in from_users:
                return await event.answer("هذه ليست لك نصب KYnX بنفسك من @KYnX0", alert=True)
            if owner and event.sender_id not in owner_and_sudos():
                return await event.answer(f"هذا هو بوت KYnX الخاص بـ {OWNER} تابعنا @KYnX0")
            try:
                await func(event)
            except Exception as er:
                LOGS.exception(er)
            ...
            if owner and event.sender_id not in owner_and_sudos():
                IN_BTTS = [
                    [
                        Button.url(
                            "قناة السورس",
                            url="https://t.me/KYnX2",
                        ),
                        Button.url("مجموعة KYnX ", url="https://t.me/KYnX3"),
                    ]
                ]
